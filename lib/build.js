...
const { directoryExists, fileExists, readJsonFile } = require("./utils");
...
async function buildInputs() {
  ...
  if (!fileExists(catTemp.path)) {
    fs.writeFileSync(catTemp.path, JSON.stringify(catTemp.data, null, 0));
    newKeys.push({
      id: (keyId++).toString(),
      value: catCfg.title,
      type: "category",
  ...
  if (secTemp.data.type === "create") {
    ...
    let currentSection = catFile.sections.find(
      (section) => section.id === secTemp.data.id
    );
    let updatedSection = {
      ...currentSection,
      articles: currentSection.articles.push({
        id: artTemp.data.id,
        title: artCfg.title,
      }),
    };
